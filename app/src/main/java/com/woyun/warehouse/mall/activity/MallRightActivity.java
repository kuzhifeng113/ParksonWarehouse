 ...
 * 商城 左侧进入的页面  查看更多
 */
public class MallRightActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mall_right);
...
//        }
        mallLeftAdapter = new MallLeftAdapter(MallRightActivity.this, listDatas, isVip);
...
//    }
    ...
    private void getData(int goodsPackId) {
        ModelLoading.getInstance(MallRightActivity.this).showLoading("", true);
        //获取数据
        try {
            JSONObject params = new JSONObject();
            params.put("goodsPackId", goodsPackId);
            RequestInterface.goodsPrefix(MallRightActivity.this, params, TAG, ReqConstance.I_GOODS_PACK_DETAIL, 1, new HSRequestCallBackInterface() {
                @Override
                public void requestSuccess(int funcID, int reqID, String reqToken, String msg, int code, JSONArray jsonArray) {
                    ModelLoading.getInstance(MallRightActivity.this).closeLoading();
                    tokenTimeLimit(MallRightActivity.this, code);
                    if (code == 0) {
                        try {
                            Gson gson = new Gson();
                            ...
                            tvPacklistName.setText(packListBean.getName());
                            tvPacklistTitle.setText(packListBean.getTitle());
                            tvPacklistSize.setText(packListBean.getShowNum() + "件商品");
                            listDatas.addAll(goodsList);
                            mallLeftAdapter.notifyDataSetChanged();
                            tvTitle.setText(packListBean.getName());
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
//                            Log.e(TAG, "requestSuccess: " + jsonResult);
                    } else {
                        ToastUtils.getInstanc(MallRightActivity.this).showToast(msg);
                    }
                }
                ...
                public void requestError(String s, int i) {
                    ModelLoading.getInstance(MallRightActivity.this).closeLoading();
                    ToastUtils.getInstanc(MallRightActivity.this).showToast(s);
                }
            });
        } catch (Exception e) {
            e.printStackTrace();
