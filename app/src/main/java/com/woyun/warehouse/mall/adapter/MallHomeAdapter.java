...
public class MallHomeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MallHomeAdapter(Context context, List<MallHomeBean.PackListBean> listBeanList) {
...
//                        left.putExtra("packlist_bean", packListBean);
                ...
                break;
            case 1:
                TwoViewHolder oneViewHolder = (TwoViewHolder) holder;
                oneViewHolder.tv_title.setText(packListBean.getTitle());
                final List<MallHomeBean.PackListBean.GoodsListBean> goodsListOne = packListBean.getGoodsList();
                RecyclerView recyclerViewOne = oneViewHolder.recycler_view_two;
                MallTypeOneAdapter mallTypeOneAdapter=new MallTypeOneAdapter(context,goodsListOne,packListBean.getShowNum());
                recyclerViewOne.setLayoutManager(new LinearLayoutManager(context));
                if(recyclerViewOne.getItemDecorationCount()==0){
                    recyclerViewOne.addItemDecoration(new SpacesItemDecoration(DensityUtils.dp2px(context, 30)));//垂直间距
                }
                recyclerViewOne.setAdapter(mallTypeOneAdapter);

                //查看更多
                oneViewHolder.tv_query_all.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                ...
                //item  点击
                mallTypeOneAdapter.setOnTypeItemClickListener(new MallTypeOneAdapter.OnTypeItemClickListener() {
                    @Override
