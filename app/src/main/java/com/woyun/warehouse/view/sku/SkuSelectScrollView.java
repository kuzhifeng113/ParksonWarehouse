...
public class SkuSelectScrollView extends SkuMaxHeightScrollView implements SkuItemLayout.OnSkuItemSelectListener {
    ...
    public void setSkuList(List<SkuListBean> skuList) {
        ...
        if (skuList.size() == 1) {
            ...
            for (SkuAttribute attribute : this.skuList.get(0).getSpecMap()) {
                selectedAttributeList.add(new SkuAttribute(attribute.getSpecName(), attribute.getSpecValue()));
            }
        }
        // 清除所有选中状态
     ...
     */
    private Map<String, List<String>> getSkuGroupByName(List<SkuListBean> list) {
        Map<String, List<String>> dataMap = new LinkedHashMap<>();
        for (SkuListBean sku : list) {
            for (SkuAttribute attribute : sku.getSpecMap()) {
                String attributeName = attribute.getSpecName();
                String attributeValue = attribute.getSpecValue();
    ...
    private void optionLayoutEnableStatusSingleProperty() {
        ...
        for (int i = 0; i < skuList.size(); i++) {
            // 属性值是否可点击flag
            SkuListBean sku = skuList.get(i);
            List<SkuAttribute> attributeBeanList = skuList.get(i).getSpecMap();
            if (sku.getNum() > 0) {
                String attributeValue = attributeBeanList.get(0).getSpecValue();
    ...
    private void optionLayoutEnableStatusMultipleProperties() {
        for (int i = 0; i < skuContainerLayout.getChildCount(); i++) {
            ...
            for (int j = 0; j < skuList.size(); j++) {
                // 属性值是否可点击flag
                boolean flag = false;
                SkuListBean sku = skuList.get(j);
                List<SkuAttribute> attributeBeanList = sku.getSpecMap();
                // 遍历选中信息列表
                for (int k = 0; k < selectedAttributeList.size(); k++) {
                    // i = k，跳过当前属性，避免多次设置是否可点击
                    if (i == k) continue;
                    // 选中信息为空，则说明未选中，无法判断是否有不可点击的情形，跳过
                    if ("".equals(selectedAttributeList.get(k).getSpecValue())) continue;
                    // 选中信息列表中不包含当前sku的属性，则sku组合不存在，设置为不可点击
                    // 库存为0，设置为不可点击
                    if (!selectedAttributeList.get(k).getSpecValue().equals(attributeBeanList.get(k).getSpecValue())
                            || sku.getNum() == 0) {
                        flag = true;
                        break;
                    }
                ...
                // flag 为false时，可点击
                if (!flag) {
                    String attributeValue = attributeBeanList.get(i).getSpecValue();
                    itemLayout.optionItemViewEnableStatus(attributeValue);
                }
            }
        }
    ...
    private boolean isSkuSelected() {
        for (SkuAttribute attribute : selectedAttributeList) {
            if (TextUtils.isEmpty(attribute.getSpecValue())) {
                return false;
            }
        }
        return true;
     ...
     * @return
     */
    public SkuListBean getSelectedSku() {
        // 判断是否有选中的sku
        if (!isSkuSelected()) {
            return null;
        }
        for (SkuListBean sku : skuList) {
            List<SkuAttribute> attributeList = sku.getSpecMap();
     ...
     * @param sku
     */
    public void setSelectedSku(SkuListBean sku) {
        selectedAttributeList.clear();
        for (SkuAttribute attribute : sku.getSpecMap()) {
            selectedAttributeList.add(new SkuAttribute(attribute.getSpecName(), attribute.getSpecValue()));
        }
    ...
    private boolean isSameSkuAttribute(SkuAttribute previousAttribute, SkuAttribute nextAttribute) {
        return previousAttribute.getSpecName().equals(nextAttribute.getSpecName())
                && previousAttribute.getSpecValue().equals(nextAttribute.getSpecValue());
    }
    ...
    public void onSelect(int position, boolean selected, SkuAttribute attribute) {
        if (selected) {
            ...
            // 取消选中，清空保存的选中信息
            selectedAttributeList.get(position).setSpecValue("");
        }
        clearAllLayoutStatus();
        // 设置是否可点击
        optionLayoutEnableStatus();
